DESCRIPTION = "Definitions for the remote-user recipe, which creates a remote user account on the system."
LICENSE = "MIT"
...
THIS_USER = "prr_remote"
THIS_GROUP = "${THIS_USER}"
GROUPADD_PARAM:${PN} = "--system ${THIS_GROUP}"
OTHER_GROUPS = "sudo dialout audio video plugdev pixelatedrailroad users input netdev gpio i2c spi"

PASSWD_SALT = "${@bb.utils.contains('IMAGE_FEATURES', 'release', 'Diam0nd188#', 'pixeleatedrail', d)}"
PASSWD_VALUE = "${@bb.utils.contains('IMAGE_FEATURES', 'release', 'BszAk.8#LMcD', 'prrtest', d)}"
